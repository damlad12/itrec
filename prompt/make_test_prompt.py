...
for dataset_name in dataset_names:
    ...
    root_path = '/content/drive/MyDrive/RecRanker/RecRanker/conventional recommender/dataset'
    with open(f'{root_path}/{dataset_name}/mov_item.pkl', "rb") as file:
        cm_item = pickle.load(file)
    with open(f'{root_path}/{dataset_name}/mov_user.pkl', "rb") as file:
        cm_user = pickle.load(file)
    with open(f'{root_path}/{dataset_name}/mov_pred.pkl', "rb") as file:
    ...
    for model_name in model_names:
        ...
        idd = '1'
        rec_list = pd.read_csv(f'{root_path}/{dataset_name}/model_result/{model_name}rec_save_dict{idd}.csv', header=None,
                               names=['v' + str(i) for i in range(11)])
        gt_list = pd.read_csv(f'{root_path}/{dataset_name}/model_result/{model_name}gt_save_dict{idd}.csv', header=None,
                              names=['u', 'i'])
        ...
        if dataset_name == 'ml-100k':
            df_like = pd.read_csv(f'{root_path}/{dataset_name}/train_set.txt', names=['u', 'i', 'r', 't'], sep=' ')
            df_dislike = pd.read_csv(f'{root_path}/{dataset_name}/dislike.txt', header=None, names=['u', 'i', 'r', 't'])
            movie_info = pd.read_csv(f'{root_path}/{dataset_name}/movie_info.csv', header=None,
                                     ...
                                     encoding='latin-1')
            df_like_p = pd.read_csv(f'{root_path}/{dataset_name}/train_set_prediction.csv', usecols=[0, 1, 2, 3])  # pointwise
        ...
        for idx, row in rec_list.iterrows():
            ...
            for ii in range(0, kk, 1):
                ...
                try:
                    ...
                    if cm_pred[uu][yy] > cm_pred[uu][yy]:
                        cm_label = 'Yes.'
                    else:
                        cm_label = 'No.'
                except Exception:
            ...
            for j_ in testlist_:
                try:
                    ...
                    uu = cm_user.get(str(uni), None)
                    # if yy is None or uu is None:
                    #     raise ValueError(f"Invalid indices: yy={yy}, uu={uu}")
        ...
        with jsonlines.open(
                f'{root_path}/{dataset_name}/test_{dataset_name}_{model_name}_pointwise.jsonl',
                mode='w') as writer:
            writer.write_all(mes_list_pointwise)
        with jsonlines.open(
                f'{root_path}/{dataset_name}/test_{dataset_name}_{model_name}_pairwise.jsonl',
                mode='w') as writer:
            writer.write_all(mes_list_pairwise)
        with jsonlines.open(
                f'{root_path}/{dataset_name}/test_{dataset_name}_{model_name}_pairwise_inv.jsonl',
                mode='w') as writer:
            writer.write_all(mes_list_pairwise_inv)
        with jsonlines.open(
                f'{root_path}/{dataset_name}/test_{dataset_name}_{model_name}_listwise.jsonl',
                mode='w') as writer:
            writer.write_all(mes_list_listwise)

        df = pd.DataFrame(my_ran_list)
        df.to_csv(f'{root_path}/{dataset_name}/my_enire_test_list{model_name}.txt', header=None, index=None)
