...
class MixGCF(GraphRecommender):
    def __init__(self, conf, training_set,valid_set, test_set):
        super(MixGCF, self).__init__(conf, training_set,valid_set, test_set)
    ...
    def train(self):
        ...
        cm_pred = torch.matmul(self.user_emb, self.item_emb.transpose(0, 1))
        with open(f"{self.dataset_path}/rating_matrix_GCF.pkl", "wb") as f:
            pickle.dump(cm_pred.detach().cpu().numpy(), f)
        self.save_mappings()
    ...
    def save_mappings(self):
        ...
        cm_user = {str(user_id): idx for idx, user_id in enumerate(self.data.user)}  # User ID to index
        cm_item = {str(item_id): idx for idx, item_id in enumerate(self.data.item)}  # Item ID to index
    
        # Generate predictions and embeddings
        cm_user_emb = self.user_emb.detach().cpu().numpy()  # User embeddings
        cm_item_emb = self.item_emb.detach().cpu().numpy()  # Item embeddings
        cm_pred = torch.matmul(self.user_emb, self.item_emb.transpose(0, 1)).detach().cpu().numpy()  # Prediction matrix
        os.makedirs(f"{self.dataset_path}model_res_pkl", exist_ok=True)
        
        # Save the user mapping
        with open(f"{self.dataset_path}model_res_pkl/mov_user.pkl", "wb") as file:
            pickle.dump(cm_user, file)
    
        # Save the item mapping
        with open(f"{self.dataset_path}model_res_pkl/mov_item.pkl", "wb") as file:
            pickle.dump(cm_item, file)
    
        # Save the prediction matrix
        with open(f"{self.dataset_path}model_res_pkl/mov_pred.pkl", "wb") as file:
            pickle.dump(cm_pred, file)
    
        # Save the user embeddings
        with open(f"{self.dataset_path}model_res_pkl/mov_user_emb.pkl", "wb") as file:
            pickle.dump(cm_user_emb, file)
    
        # Save the item embeddings
        with open(f"{self.dataset_path}model_res_pkl/mov_item_emb.pkl", "wb") as file:
            pickle.dump(cm_item_emb, file)
