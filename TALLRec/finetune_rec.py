...
def train(
    ...
    elif train_data_path.endswith(".jsonl"):  # Support for JSONL files
    ...
    elif val_data_path.endswith(".jsonl"):  # Support for JSONL files
        val_data = load_dataset("json", data_files={"validation": val_data_path}, split="train")
    else:
    ...
    # dataset_path = "/content/drive/MyDrive/EC523DL/RecRanker/RecRanker/conventional recommender/dataset/ml-100k//ml-100k_pointwise.jsonl"
    
    # # Load one example from the dataset
    # with open(dataset_path, "r") as f:
    #     for line in f:  # Read the first line
    #         example = json.loads(line)
    #         break

    # # Print the extracted example
    # print("Example:", json.dumps(example, indent=4))

    # messages = example["messages"]
    # user_message = None
    # for msg in messages:
    #     if msg["role"] == "user":
    #         user_message = "".join([content["content"] for content in msg["content"]])
    #         break
    ...
    # output = model.base_model.generate(input_ids, max_new_tokens=1, eos_token_id=tokenizer.eos_token_id)
    ...
    #     hits = (labels.unsqueeze(-1) == topk_indices).any(dim=-1).float()
    #     hit_ratio = hits.mean().item()
    ...
    #     # Check if the true labels are in the top-k
    #     hits = (labels.unsqueeze(-1) == topk_indices).any(dim=-1).float()
    #     return hits.mean()  # Average hit ratio across the batch
    ...
    trainer = transformers.Trainer(
        ...
        args=transformers.TrainingArguments(
            ...
            # metric_for_best_model="hit_ratio",
