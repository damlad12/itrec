...
def train(
    ...
    if train_data_path.endswith(".json"):  # todo: support jsonl
        ...
        print("train data not in json format")
    ...
    if train_data_path.endswith(".json") or train_data_path.endswith(".jsonl"):
        print("loading train data ")
        train_data = load_dataset("json", data_files=train_data_path)
        limited_train_data = list(train_data.take(200))  # Take the first 200 samples
    else:
        print("train data not in json format")
        train_data = load_dataset(train_data_path)
        limited_train_data = list(train_data.take(200))
    '''
    ...
    #train_data["train"] = train_data["train"].shuffle(seed=seed).select(range(200))
